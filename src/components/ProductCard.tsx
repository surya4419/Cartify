...
import { Product } from '../lib/serpapi'
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 hover:shadow-lg transition-all duration-300 group relative overflow-hidden">
      ...
      <div className="p-2">
        ...
        {/* Product Name */}
        <h3 className="font-semibold text-gray-900 mb-1 line-clamp-2 hover:text-indigo-600 transition-colors cursor-pointer text-xs leading-tight">
          {product.name}
        </h3>
        ...
        <button
          ...
          className={`w-full py-2 px-2 rounded-lg font-semibold text-xs transition-all duration-200 flex items-center justify-center space-x-1 ${product.stock_quantity === 0
            ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
            : 'bg-indigo-600 text-white hover:bg-indigo-700 active:scale-95 shadow'
            }`}
        >
        ...
        <div className="mt-2 text-xs text-gray-600 text-center">
          <div className="font-semibold text-emerald-600">Free delivery tomorrow</div>
          <div className="text-indigo-600 font-medium">Pickup available</div>
        </div>
      </div>
    </div>
  )
