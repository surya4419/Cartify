...
export const FloatinggoCartButton: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-6 lg:bottom-8 right-4 sm:right-6 lg:right-8 z-40">
      {/* Enhanced Tooltip */}
      {showTooltip && (
        <div className="absolute bottom-16 sm:bottom-20 right-0 bg-white rounded-xl shadow-2xl border-2 border-indigo-500 p-3 sm:p-4 w-72 sm:w-72 animate-bounce-slow">
          <div className="flex items-start justify-between">
            <div className="flex items-start space-x-3">
              <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-r from-indigo-600 to-purple-700 rounded-full flex items-center justify-center flex-shrink-0">
          ...
          {/* Arrow */}
          <div className="absolute bottom-0 right-6 sm:right-8 transform translate-y-1/2 rotate-45 w-3 h-3 sm:w-4 sm:h-4 bg-white border-r-2 border-b-2 border-indigo-500"></div>
        </div>
      )}
      ...
      >
        ...
        <div className="absolute inset-0 bg-indigo-600 rounded-full animate-ping opacity-10 animation-delay-1000"></div>
